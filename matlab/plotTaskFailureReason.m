function [] = plotTaskFailureReason(baseDir, folderNum, baseFileName, withError, outputDir)
    ...
    plotGenericResult(5, 5, 'Failed Tasks due to WLAN failure (%)', 'ALL_APPS', 'percentage_for_all', withError, folderNum, baseDir, strcat(baseFileName, 'Failed_Task_due_to_WLAN'), outputDir);
    
    plotGenericResult(5, 7, 'Failed Tasks due to WAN failure (%)', 'ALL_APPS', 'percentage_for_all', withError, folderNum, baseDir, strcat(baseFileName, 'Failed_Task_due_to_WAN'), outputDir);
